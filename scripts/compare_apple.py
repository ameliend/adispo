#!/usr/bin/env python3
"""
compare_apple.py — Compare les titres Apple TV+ en base Supabase avec un nouveau fichier JSON.

Affiche les titres ajoutés et supprimés, et génère :
  - scripts/apple_toadd.sql    : INSERT pour les nouveaux titres
  - scripts/apple_todelete.sql : DELETE pour les titres disparus

Usage :
    python scripts/compare_apple.py <nouveau-fichier.json>

Le nouveau fichier JSON doit contenir des entrées au format :
    { "title": "...", "platform": "Apple TV+", "status": "available",
...
def fetch_from_supabase(url, anon_key):
    endpoint = (
...
def load_new_file(path):
    ...
    canal_entries = [
        e for e in data
        if e.get('platform', '').upper().replace(' ', '') in ('APPLETV+', 'APPLETV')
    ]
    ignored = len(data) - len(canal_entries)
    if ignored:
        print(f"  ℹ️  {ignored} entrée(s) non-Apple TV+ ignorée(s)")
    return canal_entries


def generate_toadd(added_titles, existing_titles):
    lines = [
        '-- apple_toadd.sql — Nouveaux titres Apple TV+ à ajouter dans Supabase\n',
        '-- Généré par scripts/compare_apple.py\n\n',
    ]
    ...
    lines.append('-- ── Nouveaux statuts Apple TV+ ─────────────────────────────────\n\n')
...
def generate_todelete(removed_titles):
    lines = [
        '-- apple_todelete.sql — Titres Apple TV+ à supprimer de Supabase\n',
        '-- Généré par scripts/compare_apple.py\n\n',
        '-- ── Suppression des statuts Apple TV+ ──────────────────────────\n\n',
    ]
...
def main():
    ...
    print(f"Récupération des titres Apple TV+ depuis Supabase…")
    current = fetch_from_supabase(supabase_url, anon_key)
    print(f"  → {len(current)} titre(s) trouvé(s) en base\n")

    print(f"Chargement de {new_path}…")
    new_entries = load_new_file(new_path)
    print(f"  → {len(new_entries)} entrée(s) Apple TV+ dans le fichier\n")
    ...
    if added:
        ...
        out = SCRIPT_DIR / 'apple_toadd.sql'
        out.write_text(sql, encoding='utf-8')
        print(f"✓ {out} généré")

    if removed:
        sql = generate_todelete(sorted(removed, key=lambda x: x['title']))
        out = SCRIPT_DIR / 'apple_todelete.sql'
        out.write_text(sql, encoding='utf-8')
        print(f"✓ {out} généré")
