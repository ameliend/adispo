#!/usr/bin/env python3
"""
compare_netflix.py — Compare les titres Netflix en base Supabase avec un nouveau fichier JSON.

Affiche les titres ajoutés et supprimés, et génère :
  - scripts/netflix_toadd.sql    : INSERT pour les nouveaux titres
  - scripts/netflix_todelete.sql : DELETE pour les titres disparus

Usage :
    python scripts/compare_netflix.py <nouveau-fichier.json>

Le nouveau fichier JSON doit contenir des entrées au format :
    { "title": "...", "platform": "Netflix", "status": "available",
...
def fetch_netflix_from_supabase(url, anon_key):
    """Récupère tous les titres Netflix depuis Supabase via REST API."""
    endpoint = (
...
def load_new_file(path):
    """Charge le nouveau fichier JSON, filtre les entrées Netflix."""
    with open(path, encoding='utf-8') as f:
        data = json.load(f)

    netflix_entries = [
        e for e in data
        if e.get('platform', '').upper().replace(' ', '') in ('NETFLIX',)
    ]
    ignored = len(data) - len(netflix_entries)
    if ignored:
        print(f"  ℹ️  {ignored} entrée(s) non-Netflix ignorée(s)")
    return netflix_entries
...
def generate_toadd(added_titles, existing_titles):
    lines = [
        '-- netflix_toadd.sql — Nouveaux titres Netflix à ajouter dans Supabase\n',
        '-- Généré par scripts/compare_netflix.py\n\n',
    ]
    ...
    # Nouveaux statuts Netflix
    lines.append('-- ── Nouveaux statuts Netflix ────────────────────────────────────\n\n')
...
def generate_todelete(removed_titles):
    lines = [
        '-- netflix_todelete.sql — Titres Netflix à supprimer de Supabase\n',
        '-- Généré par scripts/compare_netflix.py\n\n',
        '-- ── Suppression des statuts Netflix ─────────────────────────────\n\n',
    ]
...
def main():
    ...
    print(f"Récupération des titres Netflix depuis Supabase…")
    current = fetch_netflix_from_supabase(supabase_url, anon_key)
    print(f"  → {len(current)} titre(s) trouvé(s) en base\n")

    print(f"Chargement de {new_path}…")
    new_entries = load_new_file(new_path)
    print(f"  → {len(new_entries)} entrée(s) Netflix dans le fichier\n")

    # Comparaison par titre (Netflix = une seule plateforme donc titre suffit)
    ...
    if added:
        ...
        out = SCRIPT_DIR / 'netflix_toadd.sql'
        out.write_text(sql, encoding='utf-8')
        print(f"✓ {out} généré")

    if removed:
        sql = generate_todelete(sorted(removed, key=lambda x: x['title']))
        out = SCRIPT_DIR / 'netflix_todelete.sql'
        out.write_text(sql, encoding='utf-8')
        print(f"✓ {out} généré")
